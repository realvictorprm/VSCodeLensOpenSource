...
[<AutoOpen>]
module Config =
    let [<Literal>] codeLensOutputString = "E721F849-446C-458C-997A-99E14A04CFD3"
    let fsharpOutputGuid = Guid codeLensOutputString
...
type [<Export>] Logger [<ImportingConstructor>]
    ...
    let createPane () =
        outputWindow |> Option.iter (fun x -> 
            x.CreatePane(ref fsharpOutputGuid,"Code Lens Service", Convert.ToInt32 true, Convert.ToInt32 false) |> ignore)
    ...
    member self.Log (msgType:LogType,msg:string) =
        ...
        match self.CodeLensLoggingPane, msgType with
        | None, _ -> ()
